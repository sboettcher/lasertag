// original at https://github.com/Nkawu/TFT_22_ILI9225
// some changes to make it work on edison (using mraa for spi/gpio, changing arduino stuff to normal c++)
...
TFT_22_ILI9225::TFT_22_ILI9225(int ledpin, int rstpin, int rspin) {
	...
	// init register select gpio
	_rs = new mraa::Gpio(rspin);
	if (_rs == NULL) {
		mraa::printError(MRAA_ERROR_UNSPECIFIED);
	}
	response = _rs->dir(mraa::DIR_OUT);
	if (response != MRAA_SUCCESS) {
		mraa::printError(response);
	}
}
...
bool TFT_22_ILI9225::_bitRead8(uint8_t data, uint8_t index) {
...
void TFT_22_ILI9225::drawText(uint16_t x, uint16_t y, std::string s, uint16_t color) {
