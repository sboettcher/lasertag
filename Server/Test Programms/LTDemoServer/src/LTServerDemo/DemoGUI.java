...
/**
 * A class implmenting a little test server with GUI for a laser tag game.
 * 
 * @author Marc
 */
public class DemoGUI extends javax.swing.JFrame {
    ...
    /**
     * Constructor. Creates new form DemoGUI and initializes it.
     */
    public DemoGUI() {
        initComponents();
        // Center the window in middle of the screen.
        ...
        // Initialize the GUI and some members.
        playerCounter = 1; 
    ...
    /**
     * Send data to a given player.
     * 
     * @param data - the data which should be send.
     * @param playerNr - the number of the which should receive the data.
     */
    public void sendData(String data, int playerNr) {
        // Append a newLine at end of the data.
    ...
    /**
     * Handels a new hit data input from a player.
     * 
     * @param data - the data input.
     * @param playerNr - the number of the player which sends the data.
     */
    public void newData(String data, int playerNr) {
        // Check if the data is not empty.
        if(!data.equals("")) {
            // Get the hit player and the hidden player.
            Player hiddenPlayer = Players.get(playerNr);
            Player shootPlayer = getPlyerByShootId(Integer.parseInt(data));
            // Inform the hidden player.
            sendData("" + shootPlayer.name, hiddenPlayer.playerNr - 1);
            // Decrease the health of the hidden player.
            if (hiddenPlayer.health - 25 < 0) {
                hiddenPlayer.health = 0;
            } else {
                hiddenPlayer.health = hiddenPlayer.health - 25;
            }
            // And show the new health in the GUI if necessary.
            if (this.jComboBoxSelectPlayer.getSelectedIndex() == hiddenPlayer.playerNr) {
                this.jLabelPlayerStats_HealthIn.setText("" + hiddenPlayer.health + "/100");
                this.jProgressBarPlayerStats_Health.setValue(hiddenPlayer.health);
            }
            ...
            news = news.concat(hiddenPlayer.name);
            news = news.concat(" (");
            news = news.concat(hiddenPlayer.team);
            news = news.concat(") ");
            news = news.concat(" on the head!");
            // Show the news message.
            this.jLabelNews.setText(news);
            // And schedule that will disappear after 5 sec.
            if (newsTimer != null) {
                newsTimer.cancel();
            }
            newsTimer = new Timer();
    ...
    /**
     * Retruns the player object with a given vest number/id.
     * 
     * @param id - the id.
    ...
    public Player getPlyerByShootId(int id) {
        ...
        for (int i = 0; i < Players.size(); i++) {
            // If the id matches, retrun the player.
            if (Players.get(i).westNr == id) {
                return Players.get(i);
            }
        }
        // If no players matches, return null.
        return null;
    }
    
    /**
     * Handels a new connection request.
     * 
     * @param s - the new connection as socket.
     */
    public void setSocket(Socket s) {
        ...
        // Try to open an input and an output stream.
        ...
        // Get the plyer number.
        currentNewPlayer.playerNr = playerCounter;
        // Display the new player on the GUI.
        ...
        rThread.start();
        // Set the given properties to the new player.
    ...
    /**
     * Is called if connection request is canceled.
     */
    ...
    /**
     * Handler for a selection change in player list combobox.
     * 
     * @param evt - the event.
     */
    private void jComboBoxSelectPlayerItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jComboBoxSelectPlayerItemStateChanged
        ...
        // If the number is 0, no player is selected -> clear the fields.
        if (itemNr == 0) {
            ...
            // If the number is not 0, fill the fields with the players
            // properties.
    ...
    /**
     * Handler for a click on the add player button.
     * 
    ...
    private void jButtonAddPlayerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAddPlayerActionPerformed
        ...
        // Get the given properties for the player and save them.
        ...
        // Start the waiting for a new connection.
    ...
    // MEMBERS:
    private int playerCounter;  // Curret number of players.
    private Player currentNewPlayer; // Current new player.
    private ConnectionWaiter cw;  // The connection waiter.
    private ServerSocket serverSocket;  // The general server socket.
    private WaitForCon waitForCon;  // The wiating window.
    private LT_ServerReadThread rThread;  // The read thread.
    private ArrayList<Player> Players;  // The list of players.
    private Timer newsTimer;  // The news Timer.
